...
| Qwen Code | `gemini-extension.json`, `AGENTS.md`, `commands/`, `skills/`, `hooks/qwen-hooks.json` | Installs through the Gemini manifest: `qwen extensions install DietrichGebert/ponytail:ponytail`. The `:ponytail` suffix is required — `.claude-plugin/marketplace.json` makes a bare source read as a Claude marketplace, and the installer then blocks on an interactive plugin picker (silently, with no TTY). The conversion emits `qwen-extension.json` with `contextFileName: AGENTS.md` for always-on rules, migrates `commands/*.toml` to Markdown, and auto-discovers `skills/`. Full hook tier: copy the `hooks` object from `hooks/qwen-hooks.json` into `~/.qwen/settings.json`, because Qwen's extension manifest has no `hooks` field and expands neither `${CLAUDE_PLUGIN_ROOT}` nor `${PLUGIN_DATA}`. `SessionStart` activation, `UserPromptSubmit` level tracking, and `SubagentStart` injection all work; mode state lives in `~/.qwen/.ponytail-active` (host detected via `QWEN_CODE`, which Qwen exports process-wide so the statusline sees it too) and the statusline nudge targets `ui.statusLine` in `~/.qwen/settings.json`. Qwen also reads `AGENTS.md` natively, so the instruction tier works from the repo root with zero setup — and inside this repo the extension's context copy duplicates it, so disable the extension for this workspace. |
